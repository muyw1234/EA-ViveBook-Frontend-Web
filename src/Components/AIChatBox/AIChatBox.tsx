...
const initialMessages: ChatMessage[] = [
    {
        id: 1,
        role: "assistant",
        content: "Hola, soy el asistente de recomendaciones. Dime qué tipo de libro estás buscando y revisaré los libros disponibles."
    }
];
...
const AIChatBox: React.FC = () => {
    ...
    const handleSendMessage = async (event: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || error.response?.data?.error?.message || "No se pudo contactar con el servicio de IA.";
                    ...
                    {messages.map((message) => (
                        <article key={message.id} className={`ai-message ${message.role}`}>
                            <div className="ai-message-bubble">
                                ...
                                {message.model && (
                                    <span className="ai-message-meta">Modelo: {message.model}</span>
                                )}
